class Strings {
  static String botique = "Botique";

  static String login = "Login";
  static String password = "Senha";
  static String insertPassword = "Digite sua senha";
  static String insertLogin = "Digite o Login";
  static String insertAText = "Digite o Texto";
  static String ok = "ok";
  static String cancel = "cancelar";
  static String loginError = "Não foi possível realizar o login";
  static String singUp = "Cadastre-se";
  static String logout = "Logout";
  static String email = "Email";
  static String insertEmail = "Digite o Email";
  static String name = "Nome";
  static String insertName = "Digite o nome";
  static String invalidName = "Nome Inválido";
  static String invalidEmail = "Email Inválido";
  static String invalidPassword = "A senha deve conter no mínimo 6 dígitos";
  static String posts = "Postagens";
  static String newsletters = "novidades";

  static String newsError = "Não foi possível carregar as notícias";

  static String standUser = "Felipe J. Frazão";
  static String standName = "Star Platinum";
  static String standUserEmail = "felipe11xx@hotmail.com";

  static String userCreated = "Usuário criado com sucesso";
  static String userNotCreated = "Não foi possivel concluir o cadastro";
  static String userNotCreatedError = "Não foi possivel criar um usuário";

  static String createComment = "Insira seu comentario abaixo ";
  static String comment = "Comentario";

  static String commentsCollection = "comments";

  static String defaultPicture =
      "https://i.pinimg.com/474x/6a/f3/29/6af32952636b69c6b3a3fb115ae1eb43.jpg";
  static String delete = "Apagar";
  static String edit = "Editar";

  static String wantDeleteComment = "Deseja apagar este comentário";
}
